...
class Slug extends Behavior
{
    ...
    public function getSlug($event)
    {
        ...
        $owner = $this->owner;
        $slug = $this->transliteration($owner->{$this->in_attribute});
        $result = $slug;
        if ($result != $owner->{$this->out_attribute}) {
            $i = 1;
            while ($owner::find()->where(['url' => $result])->exists()) {
                $result = $slug . '-' . $i;
                $i++;
            }
        }
        $owner->{$this->out_attribute} = $result;
    }
}